 ...
 *
 * @param {String|Object} fq -
 *
 * @return  {Query}
 * @api public
 */

Query.prototype.fq = function (fq, connector) {
  var self = this;
  var conn = connector || 'AND';
  var parameter = 'fq=';
  if (typeof (fq) === 'string') {
    parameter += encodeURIComponent(fq);
  } else {
    parameter += encodeURIComponent(fq.join(' ' + conn + ' '));
  }
  this.parameters.push(parameter);
  return self;
}
