...
Client.prototype.setWebsolrAdvancedAuth = function(token) {
  this.websolrAuth.useAdvancedAuth = true;
  this.websolrAuth.authToken = token;
}
...
function postJSON(params,callback){
   ...
   // Add the params headers if exists
   if (params.headers) {
      headers = Object.assign({}, headers, params.headers);
   }

   // If use WebSolr Advanced Auth => Add required headers
   if (params.websolrAuth.useAdvancedAuth) {
      var authHeaders = getWebsolrAuthHeaders(params.websolrAuth.authToken);
...
// Create Random String - For WebSolr Nonce
...
function getWebsolrAuthHeaders(secret) {
  var time = Math.round((new Date()).getTime() / 1000)
  var nonce = randomString(20);
  var encrypted = CryptoJS.HmacSHA1(time.toString() + nonce, secret);
  var websolr_auth = CryptoJS.enc.Hex.stringify(encrypted);

  return {
    'X-Websolr-Time': time,
    'X-Websolr-Nonce': nonce,
    'X-Websolr-Auth': websolr_auth
  };
};
